...
每一場比賽你都要做三件事：**賽前**決定先發投手與跑壘方針；**比賽中**在關鍵局面下判斷——
現在換不換投？盜壘還是等球？敬遠這一棒嗎？**賽後**看戰報，把拿到的能力點分配掉
（懶得想就按自動配置）。
...
npm test           # 78 個測試，含決定論、golden 種子、棒球規則與遊玩流程
...
| 階段 | 內容 | 狀態 |
|---|---|---|
| P1 | 種子碼、機率帳本、比賽引擎、賽前調度、關鍵決策、賽後戰報與即時分配（含自動配置）、名單、5 種結局、重播式存檔 | ✅ |
