...
`content/v1/` 一旦發佈就不再修改，新內容進 `v2/`。已存在的 id **不得刪除或改變語意**。

**為什麼**：存檔綁定 content pack 版本。舊種子永遠用舊 pack 重播。
不要假裝內容能無限往同一個 pack 加還保證等價——那是在騙自己。
...
`content/v1/names.js` 的 `BLOCKED_NAMES` 只增不減。
